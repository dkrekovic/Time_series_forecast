...
#
#     def modelSave(self):
#         return ModelCheckpoint('pinova_24h.h5', monitor='loss', mode='min', save_best_only=True)
#
#     def train(self):
...
#
#     frame = pd.concat([
#         pd.DataFrame({'day': range(len(y)), 'air_temp': y, 'type': 'original'}),
#         pd.DataFrame({'day': range(len(y)), 'air_temp': yhat, 'type': 'forecast'})
#     ])
#
#     frame['air_temp'] = [(x * train_std['air_temp']) + train_mean['air_temp'] for x in frame['air_temp']]
#
#     sns.lineplot(x='day', y='air_temp', ax=axes[i // 2, i % 2], data=frame, hue='type', marker='o')
#
# plt.tight_layout()
#
# plt.show()
...
#     n += len(true)
#
#     true = np.asarray([(x * train_std['air_temp']) + train_mean['air_temp'] for x in true])
#     hat = np.asarray([(x * train_std['air_temp']) + train_mean['air_temp'] for x in hat])
#
#     residual = true - hat
